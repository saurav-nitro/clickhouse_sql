-- ============================================================
-- Blinkit_MerchantKeyword_Impressions
...
--
-- Source: BlinkitMerchantKeyword × BlinkitKeyword
-- Destination: BlinkitMerchantKeyword_Impressions
--
-- Prisma model: BlinkitMerchantKeyword
--   merchantId  Int    → toString()
--   keywordId   String
--   count       Int    → avgState(toFloat64())
--
...
--   FROM BlinkitMerchantKeyword_Impressions GROUP BY ...
-- ============================================================
...
CREATE TABLE IF NOT EXISTS BlinkitMerchantKeyword_Impressions
(
    cdate      Date,
    merchantid String,
    keywordid  String,
...
CREATE MATERIALIZED VIEW IF NOT EXISTS BlinkitMerchantKeyword_Impressions_MV
TO BlinkitMerchantKeyword_Impressions
AS
WITH
    blinkitmerchantkeyword_daily AS (
        ...
        FROM BlinkitMerchantKeyword AS mk
        GROUP BY
            cdate,
            merchantid,
            keywordid
...
FROM blinkitmerchantkeyword_daily AS mk
...
-- Chunk 3
INSERT INTO BlinkitMerchantKeyword_Impressions
WITH
    blinkitmerchantkeyword_daily AS (
        SELECT
            toDate(mk.createdAt)      AS cdate,
...
WITH
    blinkitmerchantkeyword_daily AS (
        SELECT
            ...
            avg(toFloat64(mk.count))  AS item_count
        FROM BlinkitMerchantKeyword AS mk
        WHERE toDate(mk.createdAt) BETWEEN '2026-03-16' AND '2026-03-18'
        GROUP BY cdate, merchantid, keywordid
    ),
    total_items_per_keyword AS (
        SELECT keywordid, cdate, sum(item_count) AS total_items
