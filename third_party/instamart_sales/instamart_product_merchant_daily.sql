...
TRUNCATE TABLE InstamartProductMerchant_daily;
...
INSERT INTO InstamartProductMerchant_daily
SELECT
    toDate(pm.createdAt)                                               AS cdate,
    toString(p.groupId)                                             AS productid,
...
FROM InstamartProductMerchant AS pm
JOIN InstamartProduct  AS p ON pm.productId  = p.id
JOIN InstamartMerchant AS m ON pm.merchantId = m.id
...
-- Day 1
INSERT INTO InstamartProductMerchant_daily
SELECT toDate(pm.createdAt) AS cdate, toString(p.groupId) AS productid, toString(pm.merchantId) AS merchantid,
...
FROM InstamartProductMerchant AS pm JOIN InstamartProduct AS p ON pm.productId = p.id JOIN InstamartMerchant AS m ON pm.merchantId = m.id
