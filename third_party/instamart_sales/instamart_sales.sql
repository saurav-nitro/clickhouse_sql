...
-- Source: InstamartProductMerchant_daily (1 row per cdate/productid/merchantid)
-- ============================================================

CREATE TABLE IF NOT EXISTS Instamart_Sales
(
    cdate         Date,
    productid     String,
    merchantid    String,
...
-- Reads max(cdate) already in Instamart_Sales, inserts only new days.
-- Schedule this to run every day after InstamartProductMerchant_daily
-- has been refreshed.
-- ============================================================

INSERT INTO Instamart_Sales
WITH

-- Step 1: find the latest date already loaded
latest AS (
    SELECT coalesce(max(cdate), toDate('2025-01-01')) AS latest_date
    FROM Instamart_Sales
),
...
inventory_changes AS (
    ...
    FROM InstamartProductMerchant_daily AS d
    CROSS JOIN latest
